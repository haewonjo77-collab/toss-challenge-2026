...
import { JOIN_DAY_STAGES, slotsForDuration, weeksAheadLabel } from '../../data/schedule';
...
export function JoinTimesView({ onAdvance }: JoinTimesViewProps) {
  const { durationMinutes, weeksAhead } = useMeeting();
  ...
  return (
    <div className="card">
      ...
      <p className="join__step text-caption">
        {stageIndex + 1}/{JOIN_DAY_STAGES.length} 단계 · {weeksAheadLabel(weeksAhead)} {TOTAL_DAYS}일 중{' '}
        {abbreviate(days)}
      </p>
